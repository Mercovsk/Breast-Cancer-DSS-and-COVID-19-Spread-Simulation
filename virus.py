...
class Virus():
    def __init__(self, params):
        ...
        self.init_population = params["init_population"]
    ...
    def initial_population(self):
        # population = 4500
        self.num_currently_infected = self.init_infected
        self.total_num_infected = self.init_infected
        indices = np.arange(0, self.init_population) + 0.5
        self.thetas = np.pi * (1 + 5**0.5) * indices
        self.rs = np.sqrt(indices / self.init_population)
        self.plot = self.axes.scatter(self.thetas, self.rs, s=5, color=GREY)
        # patient zero
        i = 0
        while(i < self.init_infected):
    ...
    def spread_virus(self, i):
        ...
        if self.day % self.serial_interval == 0 and self.exposed_before < self.init_population:
            self.num_new_infected = round(self.r0 * self.total_num_infected)
            self.exposed_after += round(self.num_new_infected * 1.1)
            if self.exposed_after > self.init_population:
                self.num_new_infected = round((self.init_population - self.exposed_before) * 0.9)
                self.exposed_after = self.init_population
